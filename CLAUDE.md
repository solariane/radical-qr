...
Generated by `./updScreenshots.sh` — see `appstore/screenshots/MEMO-2.0.md`.
17 scenes: 7 iPhone, 5 iPad, 5 Mac. They are redrawn from the SwiftUI source
rather than captured, so **any generator layout change needs `METRICS` in
`appstore/screenshots/lib/app-ui.mjs` updated and a re-render**, or the store
listing starts showing a UI the app no longer has.
...
# App Store screenshots — drawn, not captured (full docs: appstore/screenshots/MEMO-2.0.md)
#   Each scene is a Node script writing an SVG in which the app screen is redrawn,
#   then rasterised with rsvg-convert. No simulator, no app build.
#   Text: appstore/screenshots/copy/<locale>.json (en-US source, fr-FR hand-written).
#   Drawing: appstore/screenshots/lib/app-ui.mjs mirrors the SwiftUI components at
#   the real GeneratorMetrics values — update METRICS there when the app layout changes.
#   Requires: brew install librsvg
./updScreenshots.sh                # translate + render + upload to ASC
./updScreenshots.sh --render-only  # no DeepL, no ASC
